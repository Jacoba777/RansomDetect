...
path2 = '.\\input_data_resized'  # path of folder to save images
...
label[0:(benign_total-1)] = 0
label[benign_total:imnbr] = 1

data, Label = shuffle(immatrix, label, random_state=2)
train_data = [data, Label]

img = immatrix[50].reshape(img_rows, img_cols)
plt.imshow(img)
...
nb_epoch = 1024 #default 20

# number of convolutional filters to use
nb_filters = 32
# size of pooling area for max pooling
nb_pool = 1 # Set to 2 by default
# convolution kernel size
nb_conv = 1 # Set to 3 by default
...
output_fn = function([model.layers[0].input], output_layer)
...
# changed 3, 1 to 1,1
output_image = np.rollaxis(np.rollaxis(output_image, 1, 1), 1, 1)
print(output_image.shape)
...
target_names = ['class 0(BENIGN)', 'class 1(RANSOMWARE)']
